...
func TestPhoneNumberUpdate(t *testing.T) {
...
func TestPhoneNumberGetReputationAudit(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.PhoneNumbers.GetReputationAudit(
		context.TODO(),
		"auditId",
...
func TestPhoneNumberStartReputationAudit(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.PhoneNumbers.StartReputationAudit(context.TODO(), "phoneNumber")
	if err != nil {
		var apierr *linqgo.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
